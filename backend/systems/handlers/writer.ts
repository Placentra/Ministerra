...
interface WriterTask {
	...
	name?: string;
	onDupli?: string[];
	is: 'insert' | 'insertIgnore' | 'sumUp' | 'replace' | 'delete';
	where?: string[];
	attempts?: number;
}

interface WriterProps {
	userTableChanges?: Map<string, (string | number)[]>;
	mode: string;
	tasksConfig: WriterTask[];
	redis: any;
	con: any;
}

interface WriterTaskError extends Error {
	isRetriable: boolean;
	arrs: any[];
	name: string;
	attempts: number;
...
const createdTempTablesByConn: WeakMap<any, Set<string>> = new WeakMap();
// LOGGER ----------------------------------------------------------------------
...
const DEBUG_WRITER: boolean = process.env.DEBUG_WRITER === '1';
...
const ensureSafeIdentifier = (value: any, context: string): string => {
...
const ensureSafeIdentifierList = (list: string[] = [], context: string): string[] => list.map(identifier => ensureSafeIdentifier(identifier, context));
...
const normalizeRows = (rows: any): any[][] => {
	...
	return Array.isArray(rows[0]) ? (rows as any[][]) : ([rows] as any[][]);
};
...
export async function Writer(props: WriterProps): Promise<void> {
	...
	const safeUserTableChanges: Map<string, (string | number)[]> = userTableChanges instanceof Map ? userTableChanges : new Map();
	...
	async function writeFailedFile(key: string, data: any, isRetriable: boolean = false, attempts: number = 0): Promise<void> {
		try {
			const filePath: string = path.join(RETRY_DIR, `${mode}_${key}_${isRetriable ? 'retriable' : 'non-retriable'}_${attempts}_${new Date().toISOString().replace(/[:.]/g, '-')}.json`);
			await fs.promises.mkdir(RETRY_DIR, { recursive: true });
			const payload: string = JSON.stringify(data);
			const payloadSize: number = Buffer.byteLength(payload);
	...
	async function loadFailedFiles(mode: string, tasksConfig: WriterTask[]): Promise<WriterTask[]> {
		const retryOperations: WriterTask[] = [];
		...
		const addRetryOperation = (operationTemplate: WriterTask, items: any, attempts: number): void => {
			const normalizedItems: any[][] = normalizeRows(items);
			...
			const createOperation = (): WriterTask => {
				const op: WriterTask = { ...templateMeta, arrs: [], attempts } as WriterTask;
				retryOperations.push(op);
				return op;
			};

			let targetOperation: WriterTask = retryOperations.find(op => op.name === templateMeta.name && op.attempts === attempts && op.arrs.length < MAX_RETRY_BUFFER_ROWS) || createOperation();
			let remaining: any[][] = [...normalizedItems];

			while (remaining.length) {
				const capacity: number = MAX_RETRY_BUFFER_ROWS - targetOperation.arrs.length;
				if (capacity <= 0) {
		...
		try {
			await fs.promises.mkdir(RETRY_DIR, { recursive: true });
			const files: string[] = await fs.promises.readdir(RETRY_DIR);

			// Clean up stale lock files from crashed processes
			const now: number = Date.now();
			for (const file of files) {
				if (file.includes('.lock.')) {
					const lockPath: string = path.join(RETRY_DIR, file);
					try {
						const stat: any = await fs.promises.stat(lockPath);
						if (now - stat.mtimeMs > STALE_LOCK_AGE_MS) {
							// Remove stale lock by renaming back to original (without .lock.pid suffix)
							const originalName: string = file.replace(/\.lock\.\d+$/, '');
							const originalPath: string = path.join(RETRY_DIR, originalName);
							try {
			...
			for (const file of files) {
				if (file.includes('_non-retriable_') && file.endsWith('.json')) {
					const filePath: string = path.join(RETRY_DIR, file);
					try {
			...
			const freshFiles: string[] = await fs.promises.readdir(RETRY_DIR);
			const modeFiles: string[] = freshFiles.filter(file => file.startsWith(`${mode}_`) && file.includes('_retriable_') && file.endsWith('.json')).slice(0, MAX_RETRY_FILES_PER_RUN);
			...
			for (const file of modeFiles) {
				...
				const originalPath: string = path.join(RETRY_DIR, file);
				const lockPath: string = `${originalPath}${RETRY_FILE_LOCK_SUFFIX}`;
				try {
				...
				try {
					...
					const nameMatch: RegExpMatchArray | null = file.match(new RegExp(`^${mode}_(.+)_retriable_(\\d+)_`));
					if (!nameMatch) {
						await safeUnlink(lockPath);
						continue;
					}
					const [name, attemptsRaw]: [string, number] = [nameMatch[1], Number(nameMatch[2] || '0')];
					const operationTemplate: WriterTask | undefined = tasksConfig.find(({ name: key }) => key === name);
	...
	const createBatches = (arr: any[], size: number = MAX_BATCH_SIZE): any[][] => Array.from({ length: Math.ceil(arr.length / size) }, (_, index) => arr.slice(index * size, index * size + size));
	...
	const executeTask = async (task: WriterTask, retryingFailed: boolean): Promise<boolean> => {
		...
		let normalizedArrs: any[][] = normalizeRows(arrs);
		...
		const safeTable: string = ensureSafeIdentifier(table, 'table');
		const safeName: string = ensureSafeIdentifier(name || table, 'temp table');
		const safeCols: string[] = ensureSafeIdentifierList(Array.isArray(cols) ? cols : [], `columns for ${safeTable}`);
		const safeWhere: string[] = ensureSafeIdentifierList(Array.isArray(where) ? where : [], `where columns for ${safeTable}`);
		const safeOnDupli: string[] = ensureSafeIdentifierList(Array.isArray(onDupli) ? onDupli : [], `ON DUPLICATE columns for ${safeTable}`);
		const columnDefinitions: string[] = Array.isArray(colsDef) ? colsDef : [];
		...
		const requiredCols: string[] = is.includes('insert') ? safeCols : safeWhere;
		...
		const getColumnDefinition = (column: string): string => {
			const defIndex: number = safeCols.indexOf(column);
			const definition: string | undefined = columnDefinitions[defIndex];
		...
		const buildTempTableDefinition = (): string => {
			const columnsForTemp: string[] = [...new Set([...safeCols, ...safeWhere])];
		...
		const validateRowShape = (rows: any[][]): string[] => {
			const allCols: string[] = is === 'sumUp' ? [...safeWhere, ...safeCols] : [...safeCols, ...safeWhere];
		...
		const allCols: string[] = validateRowShape(normalizedArrs);

		try {
			const connSet: Set<string> = createdTempTablesByConn.get(con) || new Set();
			const isInsertOperation: boolean = is.includes('insert');
			...
			const batchedArrs: any[][] = createBatches(normalizedArrs);
			const queries: any[] = [];

			for (const batch of batchedArrs) {
				const PHs: string = batch.map(() => `(${Array(allCols.length).fill('?').join(',')})`).join(',');

				if (isInsertOperation) {
					// INSERT PATH ----------------------------------------------------
					// Steps: build INSERT/INSERT IGNORE (+ optional ON DUPLICATE) so writes happen directly without temp-table overhead.
					const onDupliClause: string = safeOnDupli.length ? `ON DUPLICATE KEY UPDATE ${safeOnDupli.map(col => `${col}=VALUES(${col})`).join(',')}` : '';
					const insertPrefix: string = is === 'insertIgnore' ? 'INSERT IGNORE' : 'INSERT';
					const query: string = `${insertPrefix} INTO ${safeTable} (${allCols.join(',')}) VALUES ${PHs} ${onDupliClause}`.trim();
					...
					const insertTempQuery: string = `INSERT INTO temp_${safeName} (${allCols.join(',')}) VALUES ${PHs}`;
					...
					const joinCond: string = safeWhere.map(col => `t.${col}=t2.${col}`).join(' AND ');
					...
					const query: string = `UPDATE ${safeTable} t JOIN temp_${safeName} t2 ON ${joinCond} SET ${setClause}`;
			...
			const failed: any = await Querer({ task: mode, con, queries, mode: 'atomic_seq' });
			const failedPayload: any = typeof failed === 'object' && failed !== null ? failed : {};
			if (failedPayload[mode] !== undefined) {
				const writerTaskError: WriterTaskError = new Error(`Writer failed for ${safeName || safeTable}`) as WriterTaskError;
			...
			if (safeUserTableChanges.has(safeTable) && !retryingFailed) {
				...
				const lastChangesPipe: any = redis.pipeline();
	...
	async function executeTasks(tasks: WriterTask[], retryingFailed: boolean): Promise<WriterTask[]> {
		const failedTasks: WriterTask[] = [];
		const persistFailedPayload = async (...args: [string, any, boolean?, number?]): Promise<void> => {
			try {
		...
		for (const task of tasks) {
			try {
				...
				const writerTaskError: WriterTaskError = error as WriterTaskError;
				if (writerTaskError.isRetriable) {
					const nextAttempts: number = (writerTaskError.attempts || 0) + 1;
	...
	try {
		const retryOperations: WriterTask[] = await loadFailedFiles(mode, tasksConfig);
		...
		const currentTasks: WriterTask[] = tasksConfig.filter(task => Array.isArray(task?.arrs) && task.arrs.length);
		const failedTasks: WriterTask[] = await executeTasks(currentTasks, false);
