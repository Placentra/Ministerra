...
// Cap pool size: min 10, max 100, scaled by CPU count (avoids exceeding MySQL max_connections)
const DEFAULT_POOL_LIMIT = Number(process.env.DB_POOL_LIMIT) || Math.min(100, Math.max(10, Math.ceil(200 / CPU_COUNT)));
const DEFAULT_QUEUE_LIMIT = process.env.DB_POOL_QUEUE_LIMIT !== undefined ? Number(process.env.DB_POOL_QUEUE_LIMIT) : 0;
...
const buildSslOptions = () => {
	...
	const rejectUnauthorized = process.env.DB_SSL_REJECT_UNAUTHORIZED ? process.env.DB_SSL_REJECT_UNAUTHORIZED !== '0' : true;
	const ssl: { rejectUnauthorized: boolean; ca?: Buffer; cert?: Buffer; key?: Buffer } = { rejectUnauthorized };
...
function summarizeParamsSnippet(params) {
	// STRING SANITIZER ----------------------------------------------------------
...
function summarizeSql(sql) {
	...
	const text = typeof sql === 'string' ? sql : typeof sql === 'object' && typeof sql.sql === 'string' ? sql.sql : String(sql);
...
const instrumentPool = (pool, label) => {
	...
	const wrap = methodName => {
		...
		pool[methodName] = async (sql, params) => {
			...
			const exec = () => {
				const driverTimeoutMs = label === 'replica' ? DEFAULT_REPLICA_TIMEOUT_MS : DEFAULT_QUERY_TIMEOUT_MS;
				if (methodName === 'query' && driverTimeoutMs > 0) {
					const sqlText = toSqlString(sql) || sql;
					return original({ sql: sqlText, timeout: driverTimeoutMs }, params);
				}
				return original(sql, params);
			};
			const [rows, fields] = await breaker.execute(exec, context);
			return [rows, fields];
		};
	};
	wrap('execute');
...
const runOnReplicaWithFallback = async (methodName, sql, params, allowFallback) => {
...
const createRoutedMethod = methodName => {
	return async (sql, params, options) => {
		...
		const routeToReplica = opts.forceReplica || shouldUseReplica(sql, opts);
		if (routeToReplica) {
...
const closePoolQuietly = async pool => {
	try {
		await pool.end();
	} catch (error) {
...
const monitorPool = (poolGetter, label, rebuildFn) => async () => {
	try {
		const pool = poolGetter();
		const conn = await pool.getConnection();
		const underlying = pool.pool?.pool ?? pool.pool ?? null;
		const totalConnections = underlying?._allConnections?.length ?? 0;
		const idleConnections = underlying?._freeConnections?.length ?? 0;
		const connectionLimit = underlying?.config?.connectionLimit ?? config.connectionLimit;
		...
		conn.release();
		sqlLogger.info(`DB Pool Stats: ${JSON.stringify(stats)}`);
		if (stats.totalConnections > stats.connectionLimit * 0.9) {
			sqlLogger.alert(`High connection usage (${label}): ${stats.totalConnections}/${stats.connectionLimit}`);
		}
...
const backupDatabase = async (filename, type, retries = 3) => {
	try {
		const directoryPath = path.resolve(BACKUP_BASE_DIR, type);
		const backupFile = path.resolve(directoryPath, filename);

		await fsp.mkdir(directoryPath, { recursive: true });

		// Enforce strong encryption password before any plaintext is created
		const encPassword = process.env.SQL_CRYPTER || '';
		...
		const looksLikePath = path.isAbsolute(mysqldumpPath) || mysqldumpPath.includes('/') || mysqldumpPath.includes('\\') || mysqldumpPath.startsWith('.') || mysqldumpPath.endsWith('.exe');
		if (looksLikePath) {
			try {
		...
		return new Promise((resolve, reject) => {
			...
			const MAX_DUMP_STDERR_CHARS = Number(process.env.MYSQLDUMP_STDERR_MAX_CHARS || 65536);
			let stderr = '';
...
const restoreSQL = async (filename, type) => {
	try {
		const directoryPath = path.resolve(BACKUP_BASE_DIR, type),
			backupFile = path.resolve(directoryPath, filename),
			keepArtifacts = process.env.KEEP_RESTORE_ARTIFACTS === '1';
		const tmpDir = path.resolve(BACKUP_BASE_DIR, '_restore_tmp');
		await fsp.mkdir(tmpDir, { recursive: true });
		const tmpGz = path.resolve(tmpDir, `${path.basename(filename)}.${Date.now()}.gz`);
		const outSql = backupFile.endsWith('.gz') ? path.resolve(tmpDir, path.basename(backupFile).replace(/\.gz$/i, '')) : path.resolve(tmpDir, `${path.basename(backupFile)}.sql`);

		// Decrypt AES-256-GCM file with header MSBK|salt|iv|cipher|tag
		const encPath = `${backupFile}.enc`;
		...
		try {
			...
			const cipherStart = 32; // after header+salt+iv
			const cipherLen = stat.size - cipherStart - 16;
			const readStream = fs.createReadStream(encPath, { start: cipherStart, end: cipherStart + cipherLen - 1 });
			const password = process.env.SQL_CRYPTER || '';
			const key = await new Promise<Buffer>((resolve, reject) => {
				crypto.scrypt(password, salt, 32, (err, derivedKey) => {
					if (err) reject(err);
					else resolve(derivedKey);
