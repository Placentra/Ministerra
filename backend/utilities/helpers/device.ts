...
export async function registerDevice(con, userID, fingerprint) {
	...
	if (existing.length > 0) {
		...
		if (device.is_revoked) {
			const newSalt = crypto.randomBytes(32).toString('hex'),
				newDeviceKey = crypto.randomBytes(32).toString('hex');
		...
		let deviceKey = device.device_key;
		if (!deviceKey) {
	...
	// NEW DEVICE INSERT -------------------------------------------------------
	// generate salt+deviceKey, insert row, and return credentials to caller.
	const salt = crypto.randomBytes(32).toString('hex'),
		deviceKey = crypto.randomBytes(32).toString('hex');
	await con.execute(/*sql*/ `INSERT INTO user_devices (user_id, device_id, salt, device_key, fingerprint_hash) VALUES (?, ?, ?, ?, ?)`, [userID, deviceID, salt, deviceKey, fingerprintHash]);
