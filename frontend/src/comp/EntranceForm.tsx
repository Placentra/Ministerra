...
const imgMessagesMap = {
	...
	autoLogout: { header: 'Automatické odhlášení', detail: 'Z bezpečnostních důvodů jsme Tě raději odhlásili. Pro pokračování se znovu přihlaš.', image: `/icons/surely.png` },
	...
	unauthorized: {
		header: 'Autorizace nebyla úspěšná',
		detail: 'Email či heslo nesouhlasí a nebo vypršela autorizační lhůta. Opětovným přihlášením prokážeš, že jsi to skutečně ty.',
		...
		try {
			...
			if (submit && what === 'resendMail') {
				...
				try {
					...
					if (err.message === 'Network Error') return setInform(prev => ({ ...prev, networkError: true }));
					const errorData = err.response?.data;
					const errorCode = typeof errorData === 'string' ? errorData : errorData?.code;
					if (errorCode) setInform(prev => ({ ...prev, [errorCode]: true }));
			...
			if (dataValidity.every(Boolean)) {
				...
				if (isForgotPass) {
					await axios.post('/entrance', { mode: formMode, email });
					return setResendRetryCount(0), setInform(prev => ({ ...prev, mailSent: true }));
				}
				if (isResetPass) {
					const { data: payload } = await axios.post('/entrance', { mode: formMode, newPass: pass, useAuthToken: true });
				...
				if (isRevertEmail) {
					const { data: payload } = await axios.post('/entrance', { mode: formMode, pass, useAuthToken: true });
					if (typeof payload === 'string' && Object.prototype.hasOwnProperty.call(imgMessagesMap, payload)) return setInform({ [payload]: true });
