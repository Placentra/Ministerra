...
function FavouriteExpertTopics(p: any) {
	...
	const man = (a, c, i = null) => {
		...
		if (a === 'del') {
			...
			if (v.length < 3) return setInputWarn(['tooShort']);
			if (!validTopicPattern.test(v)) return setInputWarn(['invalidTopic']);
			if (arr.map(k => k.toLowerCase()).includes(v.toLowerCase())) return setInputWarn(['duplicate']);
			// Combined total limit across favs + exps
			if (getFavexTotalChars({ [c]: arr, [otherCat]: data[otherCat] || [] }) + v.length > TOPIC_TOTAL_CHARS_LIMIT) return setInputWarn(['tooLong']);
			...
			if (v.length < 3) return setInputWarn(['tooShort']);
			if (!validTopicPattern.test(v)) return setInputWarn(['invalidTopic']);
			if (arr.map((k, x) => x !== i && k.toLowerCase()).includes(v.toLowerCase())) return setInputWarn(['duplicate']);
			// Combined total limit across favs + exps with replacement
			const oldLen = String(arr[i] || '').length;
			const totalNow = getFavexTotalChars();
			if (totalNow - oldLen + v.length > TOPIC_TOTAL_CHARS_LIMIT) return setInputWarn(['tooLong']);
			arr[i] = v;
			setEditing({ cat: null, i: null });
		}
		superMan(c, arr);
			...
			{data.id && (
				<title-texts>
					...
					<p className='fs8 marBotXs mw160 lh1 marAuto'>
						Zadej alespoň 2 oblíbená konverzační témata a nebo odborná témata, v nichž rád obohatíš ostatní. Pro obě kategorie dohromady je maximální počet znaků {TOPIC_TOTAL_CHARS_LIMIT}.
					</p>
					{[...(data.favs || []), ...(data.exps || [])].reduce((t, u) => t + String(u || '').length, 0) >= TOPIC_TOTAL_CHARS_LIMIT && (
